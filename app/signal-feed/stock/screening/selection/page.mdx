...
- 推荐配合以下文档使用，形成完整研究链路：
  - 方法论：`/signal-client/methodology/indicator-guide`
  - 预设边界：`/signal-client/methodology/filter-presets`
  - 操作流程：`/signal-client/guides/stock-screener`
...
1. 从低耦合条件开始分页拉取，先看样本分布与字段质量。
2. 将结果视为“候选池”，进入二次研究而非直接执行。
3. 在消费端保留筛选版本、时间戳和关键条件，便于复盘。
